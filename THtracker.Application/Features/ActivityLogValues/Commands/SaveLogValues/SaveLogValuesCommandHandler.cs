...
public sealed class SaveLogValuesCommandHandler : IRequestHandler<SaveLogValuesCommand, Result<IEnumerable<LogValueResponse>>>
{
    ...
    public SaveLogValuesCommandHandler(
    ...
    public async Task<Result<IEnumerable<LogValueResponse>>> Handle(SaveLogValuesCommand request, CancellationToken cancellationToken)
    {
        ...
        foreach (var valReq in request.Values)
        {
            ...
            var logValue = new ActivityLogValue(request.ActivityLogId, valReq.ValueDefinitionId, valReq.Value);
